...
class Joint(pyglet.shapes.Circle):
    def __init__(self, batch, space, radius, pin, v, v_rot,
                 color, group):
        super().__init__(*(offset + v_rot), radius, color=color,
                         batch=batch, group=pyglet.graphics.Group(group))
...
class Link(pyglet.shapes.Rectangle):
    def __init__(self, batch, space, size, pin, angle, color, group):
        super().__init__(*pin.position, *size, color=color,
                         batch=batch, group=pyglet.graphics.Group(group))
        self.body = pymunk.Body()
